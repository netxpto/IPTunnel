...
bool BasisReconciliation::runBlock(void)
{
	...
	if (basisReconciliationRole == basis_reconciliation_role::Alice) {
		...
		if ((finishedReceiving) && (msgInputData.size() > 0)) {
			...
			int ready = min(inputSignals[0]->ready(), inputSignals[1]->ready());
			int space = min(outputSignals[0]->space(), outputSignals[1]->space());
			int process = min(ready, space);
			process = min(process, readyFromBob);
		...
		if (!bobBasisReconStatus) {
			...
			int ready = min(inputSignals[0]->ready(), inputSignals[1]->ready());
			int minBufSz = (int)min(outputSignals[0]->getBufferLength() - 1, outputSignals[1]->getBufferLength() - 1);
			ready = min(ready, minBufSz);
			...
			if ((finishedReceiving) && (msgInputData.size() > 0)) {
				...
				space = min(space, outputSignals[1]->space());
				int process = min((int)msgInputData.size(), space);
				if (process <= 0) return alive;
		...
		cout << "Undefined role of the basis reconcilliation block.";
	}
	else {
		cout << "Undefined behavior on basis reconcilliation block.";
	}

	return alive;
}

bool BasisReconciliation::sendToMsg(vector <t_integer>& data, Signal& signalToMsg, bool& started) {
