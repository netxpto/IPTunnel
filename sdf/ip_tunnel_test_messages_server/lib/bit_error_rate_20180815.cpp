...
bool BitErrorRate::runBlock(void){
	...
	if (process == 0)
	{
		...
		double UpperBound = BER + 1.0 / sqrt(receivedBits) * z  * sqrt(BER*(1.0 - BER)) + 1.0 / (3.0 * receivedBits)*(2.0 * z * z * (1.0 / 2.0 - BER) + (2.0 - BER));
		double LowerBound = BER - 1.0 / sqrt(receivedBits) * z  * sqrt(BER*(1.0 - BER)) + 1.0 / (3.0 * receivedBits)*(2.0 * z * z * (1.0 / 2.0 - BER) - (1.0 + BER));
		...
		ofstream myfile;
		myfile.open(outputFolderName + outputFileName);
		myfile << "BER= " << BER << "\n";
