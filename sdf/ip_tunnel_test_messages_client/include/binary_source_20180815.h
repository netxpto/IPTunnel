...
public:
	...
	BinarySource(initializer_list<Signal *> InputSig, initializer_list<Signal *> OutputSig) :
		Block(InputSig, OutputSig),
		seed { (long int)chrono::system_clock::now().time_since_epoch().count() }
	{};

	BinarySource(initializer_list<Signal *> InputSig, initializer_list<Signal *> OutputSig, long int inputSeed) :
		Block(InputSig, OutputSig),
		seed{ inputSeed }
	{};
