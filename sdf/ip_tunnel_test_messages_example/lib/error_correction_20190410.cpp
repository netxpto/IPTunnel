#include "..\include\error_correction_20190410.h"

void ErrorCorrection::initialize(void) {

	int maxInData = min(inputSignals[0]->getBufferLength(), inputSignals[1]->getBufferLength());
	int maxOutData = min(outputSignals[0]->getBufferLength(), outputSignals[1]->getBufferLength());
	maxData = min(maxOutData, maxInData);

	//	minimumSyndromeSize = min(minimumSyndromeSize, minDataCalc);
	minimumSyndromeSize = minimumSyndromeSize;



	//	keyIdx.reserve(maxData);
...
bool ErrorCorrection::runBlock(void)
{
	...
	if (errorCorrectionRole == error_correction_role::Alice) {
		...
		if (!errorCorrectionStatus) {
			...
			if (changeBerReady > 0) {
				vector<t_integer> changeBerVector{};
				bool haveBer{ false };
				t_integer numVals{ 0 };
...
//			int ready0 = inputSignals[0]->ready();
			...
			int ready1 = (int)inputSignals[1]->ready();
...
//			if (numberOfPartitions > space) return alive;
			...
			for (auto w = 0; w < (int)numberOfPartitions; w++) {
				...
				endPositions.push_back((t_integer)min(((t_real)partitionSize * w + (t_real)partitionSize), (t_real)ready0));
				//				endPositions.push_back(partitionSize * w + min(ceil((t_real)partitionSize / 2.0), (t_real)process));
				t_real leftover = (t_real)ready0 - ((t_real)partitionSize * w);
				totalSearchSpace.push_back(min(partitionSize, (t_integer)leftover));
...
//			int spaceEC = outputSignals[1]->space();
			...
			if (currentPass == 1) {
				...
				if (doingLookBack && !lookBackStarted) {
					...
					vector<t_integer> startPositions2{};
					for (auto w = 0; w < (int)numberOfPartitions; w++) {
					...
					vector<t_integer> startPositions3{};
					...
					for (auto w = 0; w < (int)numberOfPartitions; w++) {
						...
						if ((startPositions.empty()) || (startPositions.back() != startPos)) {
							...
							endPositions.push_back((t_integer)min(((t_real)startPos + (t_real)partitionSizeLB), (t_real)process));
							//	endPositions.push_back(partitionSize * w + min(ceil((t_real)partitionSize / 2.0), (t_real)process));
							t_real leftover = (t_real)process - ((t_real)startPos);
							totalSearchSpace.push_back(min(partitionSizeLB, (t_integer)leftover));
					...
					if (readyPerm == 0) return alive;

					if (doubleWindowSize && currentPartitionSize * 2 <= minimumSyndromeSize / minimumNumberOfPartitions) {

						currentPartitionSize = currentPartitionSize * 2;
					}
					...
					for (auto w = 0; w < (int)numberOfPartitions; w++) {
						...
						endPositions.push_back((t_integer)min(((t_real)currentPartitionSize * w + (t_real)currentPartitionSize), (t_real)process));
						t_real leftover = (t_real)process - ((t_real)currentPartitionSize * w);
						totalSearchSpace.push_back(min(currentPartitionSize, (t_integer)leftover));
	...
	else if (errorCorrectionRole == error_correction_role::Bob) {
...
//				outputSignals[3]->bufferPut((t_binary)1);
			...
			*/
			...
			int ready = min((t_integer)dataOnHold.size(), maximumSyndromeSize);
			//			int ready = inputSignals[0]->ready();
			int ready1 = inputSignals[1]->ready();
			int space = outputSignals[1]->space();
			int process = min((t_integer)floor(ready / partitionSize), space);
...
//			bool restart = false;
			...
			for (auto w = 0; w < numberOfPartitions; w++) {
				startPositions.push_back(partitionSize * w);
				endPositions.push_back((t_integer)min(((t_real)partitionSize * w + (t_real)partitionSize), (t_real)process));
				//			endPositions.push_back(partitionSize * w + min(ceil((t_real)partitionSize / 2.0), (t_real)process));
...
//					}
				...
				////////////////////////////////////////////////////////////////////////////
					...
					*/
					bool sentSeed = false;
					vector <t_integer> seedVector{seed};
				...
				paritiesMsgInput.erase(paritiesMsgInput.begin(), paritiesMsgInput.end());
					...
					if (noErrors) {
						// Trigger lookback
						if (((int)currentPass >= numPasses) && ((lookBackDone) || (lookBackPassesIdx[0].size() == 0))) {
							outputKey({ outputSignals[0] }, keys[0]);							currentPass = 0;
							errorCorrectionStatus = false;
							currentPassStarted = false;
						}
						else {
	...
	else if (errorCorrectionRole == error_correction_role::Undefined) {
		cout << "Undefined role of the error correction block.";
	}
	else {
		cout << "Undefined behavior on the error correction block.";
	}
...
t_integer ErrorCorrection::getParity(vector<t_integer>& keyVector, const t_integer startPosition, const t_integer endPosition, t_integer& count, vector<vector<t_integer>>& keyVectors, const vector<vector<t_integer>>& keyIdxVectors, const vector<vector<t_integer>>& reverseKeyIdxVectors, const unsigned int pass) {
	t_integer parity = 0;
...
bool ErrorCorrection::outputKey(vector<Signal*> outputSignals_f, vector<t_integer>& correctedKey) {
	bool alive = false;
	t_integer process = (t_integer)correctedKey.size();
	for (auto k = 0; k < (int)outputSignals_f.size(); ++k) {
		process = min(process, outputSignals_f[k]->space());
	}
...
bool ErrorCorrection::generatePermutationsIdx_testing(const t_integer seed, const vector <t_integer>& originalKey, vector <t_integer>& newScrambledKey, vector <t_integer>& newScrambledKeyIdx, vector <t_integer>& newReverseScrambledKeyIdx) {
	bool alive = false;
...
t_integer ErrorCorrection::ber2partitionSize(t_real qValue, t_real BERValue) {
	t_integer pSz = (int)floor(qValue / max(BERValue, minimumBER));
	return pSz;
}
